...
def plot_decision_tree(model, feature_names):
    plt.figure(figsize=(12,8))  # Ajuste o tamanho da figura para tornar a visualização compacta
    plot_tree(model, 
              feature_names=feature_names, 
              class_names=['not spam', 'spam'], 
              filled=True, 
              rounded=True,
              max_depth=3,  # Ajuste a profundidade para limitar o tamanho da árvore
...
def main():
    ...
    # Decision Tree
    dt = DecisionTreeClassifier(max_depth=3)  # Ajuste a profundidade conforme necessário
    evaluate_model_holdout(X_train, y_train, X_test, y_test, dt, "Decision Tree")

    # Plotar a árvore de decisão
    plot_decision_tree(dt, X.columns)
