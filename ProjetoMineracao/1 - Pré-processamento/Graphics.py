...
def main():
    ...
    # Histograma para a coluna 'spam'
    plt.figure(figsize=(8, 6))
    plt.hist(df['spam'], bins=20, color='blue', alpha=0.5)
    plt.title('Histograma para a coluna spam')
    plt.xlabel('Valor')
    ...
    plt.figure(figsize=(8, 6))
    plt.pie(contagem_spam, labels=contagem_spam.index, autopct='%1.1f%%', colors=['lightcoral', 'lightskyblue'])
    plt.title('Proporção de spam e não-spam')
    plt.tight_layout()
    ...
    # Gráfico de Linha para word_freq_you
    plt.figure(figsize=(10, 6))
    df['word_freq_you'].plot(kind='line')
    plt.title('Gráfico de Linha: word_freq_you')
    plt.xlabel('Índice')
    plt.ylabel('word_freq_you')
    plt.tight_layout()
    plt.savefig('Gráfico_de_Linha.png', format='png')
    plt.show()
    
    # Pair Plot para word_freq_receive, word_freq_business, word_freq_remove e spam
    plt.figure(figsize=(10, 6))
    sns.pairplot(df[['word_freq_receive', 'word_freq_business', 'word_freq_remove', 'spam']])
    plt.tight_layout()
